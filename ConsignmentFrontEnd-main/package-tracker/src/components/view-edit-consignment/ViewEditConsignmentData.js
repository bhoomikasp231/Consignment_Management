...
const handleInputChange = (event) =>{
 ...
 const handleBtnSearch = () =>{
  client.send(JSON.stringify({ 
    ...
    axios.get(`${baseUrl}/tracker/consignment/${metaDataInput.searchContent}`).then((response)=>{
        ...
        if(response?.data?.consignment?.trackStatus){
          setTrackStatus(response?.data?.consignment?.trackStatus);
          console.log('sss',response?.data?.consignment?.trackStatus);
        } else{
          setTrackStatus("");
        }
   ...
   srcPersonMail :data.consignment.sourcePerson.emailID,
    ...
    return(
        <div className="meta-data-container">
            ...
            <h3 style={{fontSize:'1.5em',textAlign:'left', width:'100%', paddingLeft:'10px', paddingTop:'10px', color:'#007259'}}>View Or Edit the Consignment details</h3>
            <div className="search-bar-layout tour-search">
            <input style={{width:'80%', borderRadius:'10em', height:'2.1em'}} type='text' placeholder='Search... By RFID number' name='searchContent' onChange={handleInputChange} ></input>
  ...
  <p>Uploading Data, please be patient.</p>
      ...
      <TrackConsignment trackStatus={trackStatus}/>
