...
const featchCategoryAndCompany = (url)=>{
    axios.get(url).then((response)=>{
      console.log('category', response.data.stockData[0].category);
      response.data.stockData[0].category.map((props, index)=>{
        setCategoryArray((prevData)=> [...prevData, <option key={index+44} >{props}</option> ]);
      });
      response.data.stockData[0].companyName.map((props, index)=>{
        setCompanyArray((prevData)=> [...prevData, <option key={index+88} >{props}</option> ]);
      });
    }).catch((error)=>{
      setTimeout(()=>{

      //  alertmsg.show('Category and company API Error', { type: types.ERROR })
      }, 500);
...
const featchData = (url) =>{
    axios.get(url).then((response)=>{
        if(response.data.unitOfMesure.length >= 1){
          console.log('data',response);
            addUnitOfMeasure(response.data.unitOfMesure);
            setLoadingVisible("spanner Notshow");
        }
    }).catch((error)=>{
      setTimeout(()=>{
     //   alertmsg.show('UOM data not loaded please check your internet or server is down please contact admin', { type: types.ERROR });
      },500);
      setLoadingVisible("spanner Notshow");
        console.log(error);
    });
...
const addUnitOfMeasure = (data) => {
    console.log('data',data);
     data.map((props, index)=> 
     {
            setUomResponse((prevData)=> [...prevData, <option key={index} >{props.uom}</option> ])
     })
